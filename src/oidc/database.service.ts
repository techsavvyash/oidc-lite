...
export class DatabaseService {
  private readonly storage = new LRUCache<any,any,unknown>({ max: 1000 });
  ...
  upsert(
    ...
    if (model === 'Session') {
      this.storage.set(this.sessionUidKeyFor(uid), id, expiresIn * 1000 as unknown);
    }
    ...
    if (userCode) {
      this.storage.set(this.userCodeKeyFor(userCode), id, expiresIn * 1000 as unknown);
    }

    this.storage.set(key, payload, expiresIn * 1000 as unknown);
  }
