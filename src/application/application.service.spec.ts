...
describe('ApplicationService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: PrismaService, useValue: PrismaServiceMock },
        {
          ...
          useValue: ApplicationRolesServiceMock,
        },
        {
          provide: ApplicationScopesService,
          useValue: ApplicationScopesServiceMock,
        },
  ...
  describe('createApplication', () => {
    ...
    it('should throw UnauthorizedException if route validation fails', async () => {
      HeaderAuthServiceMock.validateRoute.mockResolvedValue({
        success: false,
        message: 'Unauthorized',
      });
      const res = mockResponse();
      await expect(
        service.createApplication('uuid', {} as CreateApplicationDto, {}, res),
    ...
    it('should throw BadRequestException if no uuid is provided', async () => {
      HeaderAuthServiceMock.validateRoute.mockResolvedValue({
        success: true,
        data: { tenantsId: 'tenantId' },
      });
      const res = mockResponse();
      await expect(
        service.createApplication('', {} as CreateApplicationDto, {}, res),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if application already exists', async () => {
      ...
      await expect(
        service.createApplication(
          'uuid',
          { name: 'appName' } as CreateApplicationDto,
          {},
          res,
        ),
      ).rejects.toThrow(BadRequestException);
    ...
    it('should throw BadRequestException if no data is provided', async () => {
      ...
      const res = mockResponse();
      await expect(
        service.createApplication('uuid', null, {}, res),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should create throw BadRequestException if refresh token time and time to live not given', async () => {
      HeaderAuthServiceMock.validateRoute.mockResolvedValue({
        success: true,
        data: { tenantsId: 'minio-tenant' },
      });
      PrismaServiceMock.tenant.findUnique.mockResolvedValue({
        accessTokenSigningKeysId: 'null-key',
        idTokenSigningKeysId: 'key2',
        data: JSON.stringify({
          accessTokenSigningKeysID: 'key1',
          idTokenSigningKeysID: 'key2',
          refreshTokenTimeToLiveInMinutes: null,
  ...
  describe('patchApplication', () => {
    it('should throw UnauthorizedException if route validation fails', async () => {
      ...
      const res = mockResponse();
      await expect(
        service.patchApplication('id', {} as UpdateApplicationDto, {}, res),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should throw BadRequestException if no id is provided', async () => {
      const res = mockResponse();
      await expect(
        service.patchApplication(null, {} as UpdateApplicationDto, {}, res),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if no data is provided', async () => {
      ...
      const res = mockResponse();
      await expect(
        service.patchApplication('id', null, {}, res),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw UnauthorizedException if tenantId does not match and tenantsId is not null', async () => {
      HeaderAuthServiceMock.validateRoute.mockResolvedValue({
        success: true,
        data: { tenantsId: 'differentTenantId' },
      });
      PrismaServiceMock.application.findUnique.mockResolvedValue({
        id: 'id',
        tenantId: 'tenantId',
    ...
    it('should throw InternalServerError while updating the application', async () => {
      ...
      const error = new Error(
        'Unique constraint failed on the fields: (`name`,`tenantId`)',
      );
    ...
    it('should update application successfully', async () => {
      ...
      const res = mockResponse();
      await service.patchApplication(
        'id',
        {
          name: 'newName',
          oauthConfiguration: { authorizedOriginURLs: [] },
  ...
  describe('returnAllApplications', () => {
    it('should throw UnauthorizedException if authorization header verification fails', async () => {
      HeaderAuthServiceMock.authorizationHeaderVerifier.mockResolvedValue({
        success: false,
        message: 'Unauthorized',
      });
      await expect(service.returnAllApplications({})).rejects.toThrow(
        UnauthorizedException,
      );
    });
  ...
  describe('returnAnApplication', () => {
    it('should throw UnauthorizedException if route validation fails', async () => {
      ...
      await expect(service.returnAnApplication('id', {})).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw BadRequestException if no id is provided', async () => {
      ...
      await expect(
        service.returnAnApplication(null, { authorization: 'master' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if application does not exist', async () => {
      ...
      PrismaServiceMock.application.findUnique.mockResolvedValue(null);
      await expect(service.returnAnApplication('app-id', {})).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should return an application successfully', async () => {
      HeaderAuthServiceMock.validateRoute.mockResolvedValue({
        success: true,
        data: { tenantsId: 'tenant_id' },
      });
      PrismaServiceMock.application.findUnique.mockResolvedValue({
        id: 'id',
        tenantId: 'tenant_id',
      });
      const result = await service.returnAnApplication('id', {
        authorization: 'master',
      });
      expect(result.success).toBe(true);
  ...
  describe('deleteApplication', () => {
    it('should throw UnauthorizedException if route validation fails', async () => {
      ...
      await expect(service.deleteApplication('id', true, {})).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw BadRequestException if application does not exist', async () => {
      ...
      PrismaServiceMock.application.findUnique.mockResolvedValue(null);
      await expect(
        service.deleteApplication('app-id', true, {}),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('returnOauthConfiguration', () => {
    it('should throw UnauthorizedException if route validation fails', async () => {
      ...
      await expect(service.returnOauthConfiguration('id', {})).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw BadRequestException if no id is provided', async () => {
      ...
      await expect(service.returnOauthConfiguration(null, {})).rejects.toThrow(
        BadRequestException,
      );
    });
